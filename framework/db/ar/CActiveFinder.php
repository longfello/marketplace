...
class CJoinElement
{
	...
	public function getJoinCondition()
	{
      ...
      $schema=$this->_builder->getSchema();
			...
			$joinTableName=$this->relation->getJunctionTableName();
			if(($joinTable=$schema->getTable($joinTableName))===null)
				throw new CDbException(Yii::t('yii','The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database (via {connection_string}).',
					array('{class}'=>get_class($parent->model), '{relation}'=>$this->relation->name, '{joinTable}'=>$joinTableName, '{connection_string}' => $schema->dbConnection->connectionString)));
			$fks=$this->relation->getJunctionForeignKeys();
