...
class SGridView extends CGridView {
	...
	public function getDefaultActionOptions()
	{
		return array(
			...
			'data-question'=>Yii::t('SGridView', 'Выполнить действие?'),
	...
	public function getCustomActions()
	{
		if ($this->hasDeleteAction === true)
		{
			$this->customActions = array(array(
				'label'=>Yii::t('SGridView', 'Удалить'),
				'url'=>$this->owner->createUrl('delete'),
				'linkOptions'=>array(
					'class'=>'actionDelete',
					'data-question'=>Yii::t('SGridView', 'Вы действительно хотите удалить выбранные объекты?'),
				)
			));
		}
	...
	public function insertDropdownHtml()
	{
			...
			foreach ($filters as $filter)
			{
				$filtersHtml .= strtr('
					...
					'{delete}'=>CHtml::ajaxLink(
						CHtml::image($this->baseScriptUrl.'/cross.png', Yii::t('SGridView', 'Удалить')),
						...
						// Html options
						array(
							'confirm'=>Yii::t('SGridView','Вы действительно хотите удалить этот фильтр?'),
							'id'=>'fdLink'.$filter->id
						)
					)
				));
		...
		echo strtr('
			...
			<div class="gridViewOptionsMenu">
				<ul>
					<li>
						<a href="#" onClick="clearSGridViewFilter(\''.$this->getId().'\');">'.Yii::t('SGridView','Очистить фильтр').'</a>
					</li>
					<li>{saveLink}</li>
					'.$filtersHtml.'
				</ul>
			</div>', array(
				'{saveLink}'=>$this->_checkAttributes() ? '<a href="#" onClick="$(\'#'.$this->getId().'saveFilterDialog\').dialog(\'open\');">'.Yii::t('SGridView','Сохранить фильтр').'</a>':'<a class="nonActive">'.Yii::t('SGridView','Сохранить фильтр').'</a>',
			));
		echo CHtml::openTag('div', array(
		...
		echo '
			<div class="form">
				<div class="row">
					...
					<div class="hint">'.Yii::t('SGridView','Укажите имя фильтра').'</div>
				</div>
			</div>
		';
		echo CHtml::closeTag('div');
