...
class QiwiConfigurationModel extends CModel
{
	...
	public function getFormConfigArray()
	{
		...
		if($id==='undefined')
			$successUrl = Yii::t('QiwiPaymentSystem','Для получениия ссылки нужно сохранить запись.');
		else
		...
		return array(
			...
			'elements'=>array(
				'shop_id'=>array(
					...
					'type'=>'text',
					'hint'=>Yii::t('QiwiPaymentSystem','Пример: 2042'),
				),
				'password'=>array(
				...
				'<div class="row">
					<label>'.Yii::t('QiwiPaymentSystem','URL для отправки в случае успешной оплаты счёта').'</label>
					'.$successUrl.'
				</div>
				'
		));
