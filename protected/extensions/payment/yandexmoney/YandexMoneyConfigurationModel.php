...
class YandexMoneyConfigurationModel extends CModel
{
	...
	public function getFormConfigArray()
	{
		...
		return array(
			...
			'elements'=>array(
				...
				'<div class="row">
					<label>'.Yii::t('YandexMoneyPaymentSystem', 'Адрес, на который получать уведомления').'</label>
					<input type="text" value="'.$this->getCallbackUrl($id).'" disabled>
					<div class="hint">
						'.Yii::t('YandexMoneyPaymentSystem', 'Сохраните этот адрес на странице <a href="https://sp-money.yandex.ru/myservices/online.xml" target="_blank">HTTP-уведомления</a>').'
					</div>
				</div>'
		));
	}
	...
	{
		if($id==='undefined')
			return Yii::t('YandexMoneyPaymentSystem', 'Для получениия ссылки нужно сохранить запись.');
