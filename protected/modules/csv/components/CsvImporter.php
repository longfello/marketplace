...
class CsvImporter extends CComponent
{
	...
	public function validate()
	{
		...
		{
			$this->errors[]= array('line'=>0,'error'=>Yii::t('CsvModule', 'Файл недоступен.'));
			return false;
		}
		...
		foreach(array('category','name','type','price') as $column)
		{
			if(!in_array($column, $this->csv_columns))
				$this->errors[]=array('line'=>0, 'error'=>Yii::t('CsvModule', 'Укажите обязательную колонку {column}.',array('{column}'=>$column)));
		}
	...
	{
		$attributes = array(
			'type'                   => Yii::t('StoreModule', 'Тип'),
			'name'                   => Yii::t('StoreModule', 'Название'),
			'category'               => Yii::t('StoreModule', 'Категория'),
			'additionalCategories'   => Yii::t('StoreModule', 'Доп. Категории'),
			'manufacturer'           => Yii::t('StoreModule', 'Производитель'),
			'sku'                    => Yii::t('StoreModule', 'Артикул'),
			'url'                    => Yii::t('StoreModule', 'URL'),
			'price'                  => Yii::t('StoreModule', 'Цена'),
			'is_active'              => Yii::t('StoreModule', 'Активен'),
			'image'                  => Yii::t('StoreModule', 'Главное изображение'),
