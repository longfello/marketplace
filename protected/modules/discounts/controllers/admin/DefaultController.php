...
class DefaultController extends SAdminController
{
	...
	public function actionUpdate($new = false)
	{
		...
		if (!$model)
			throw new CHttpException(404, Yii::t('DiscountsModule', 'Скидка не найдена.'));
		...
		$form->additionalTabs = array(
			Yii::t('DiscountsModule','Категории') => $this->renderPartial('_categories', array('model'=>$model), true),
		);
		...
		if (Yii::app()->request->isPostRequest)
		{
			...
			{
				$model->save();
				$this->setFlashMessage(Yii::t('DiscountsModule', 'Изменения успешно сохранены'));
