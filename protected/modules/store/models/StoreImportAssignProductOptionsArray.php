 ...
 */
class StoreImportAssignProductOptionsArray extends CFormModel
{
  const EMPTY_VALUE = StoreImportAssignProductOptions::SOLUTION_NONE;
...
//              echo($key);
              $compareValue = self::EMPTY_VALUE;
//              throw new CException("Key {$key} does not exist in array!");
            } else  $compareValue = $row[$key];
          } else {
