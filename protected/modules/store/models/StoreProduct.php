...
{
  const LastViewedList = 'LastViewedList';
    ...
    return array(
			'active'=>array(
				'condition'=>$alias.'.is_active=1 AND market_id IN ('.implode(',', Yii::app()->user->location->market_ids).')',
			),
			'newest'=>array('order'=>$alias.'.created DESC'),
			'discount'=>array('condition'=>"NULLIF({$alias}.discount, '') IS NOT NULL"),
			'byViews'=>array('order'=>$alias.'.views_count DESC'),
			'byAddedToCart'=>array('order'=>$alias.'.added_to_cart_count DESC'),
      'viewed' => array('condition' => $alias.'.id IN ('.implode(',', self::getLastViewedList()).')'),
      'commented' => array(
        'select'=>'count(co.id) as commentsCount, t.*',
        'join'=>'INNER JOIN Comments co ON co.object_pk = '.$alias.".id AND class_name = '".Comment::CLASS_PRODUCT."'",
        'group'=>$alias.'.id',
	...
	public function rules()
	{
		return array(
			...
			array('quantity, availability, manufacturer_id, market_id, source_id, source_pk', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
      ...
      'source'          => array(self::BELONGS_TO, 'StoreImportSources', 'source_id'),
	 ...
	 * @return StoreProduct
	 */
	public function applyCategoriesRecursive(StoreCategory $categories, $select = 't.*')
	{
    $criteria = new CDbCriteria;
    ...
    $criteria->join = '
INNER JOIN `StoreProductCategoryRef` `categorizationRecursive` ON (`categorizationRecursive`.`product`=`t`.`id`)
INNER JOIN `StoreCategory` `cat` ON (`cat`.`id`=`categorizationRecursive`.`category`) AND cat.lft >= '.$categories->lft.' AND cat.rgt <='.$categories->rgt;
    $this->getDbCriteria()->mergeWith($criteria);
    ...
    /*
		...
		{
      die('Todo: WTF? Why not StoreCategory?');
			if(!is_array($categories)) {
    ...
    $hash = 'applyCategoriesRecursive-'.implode('-', $categories);
    $tmp = Yii::app()->cache->get($hash);
    if (!$tmp) {
      $tmp = array();
      foreach($categories as $category) {
        $cats = StoreCategory::model()->findByPk($category)->asWithChilds();
        $tmp = array_merge($tmp, $cats);
      }
      Yii::app()->cache->set($hash, $tmp, 30);
    }
    $categories = $tmp;
  ...
  public function getRate(){
    $criteria = new CDbCriteria();
    $criteria->addCondition("class_name='application.modules.store.models.StoreProduct'");
    $criteria->addCondition("status = 1");
    $criteria->addCondition("object_pk = {$this->id}");
    $criteria->select='avg(rate) as rate';

    $rating = Comment::model()->find($criteria)->getAttribute('rate');
	 ...
	 * @param $value
	 */
	public function applyRating($value)
	{
		if($value)
		{
      $value = is_array($value)?$value:explode(';', $value);
			$criteria = new CDbCriteria;
			$criteria->addInCondition('(t.rating/t.votes)', $value);
			$this->getDbCriteria()->mergeWith($criteria);
		}
		return $this;
	}
	...
	public function attributeLabels()
	{
		return array(
			'id'                     => 'ID',
			'manufacturer_id'        => Yii::t('StoreModule', 'Производитель'),
			'market_id'              => Yii::t('StoreModule', 'Магазин'),
			'source_id'              => Yii::t('StoreModule', 'id источника импорта'),
			'source_pk'              => Yii::t('StoreModule', 'id товара в источнике'),
			'type_id'                => Yii::t('StoreModule', 'Тип'),
			'use_configurations'     => Yii::t('StoreModule', 'Использовать конфигурации'),
			'name'                   => Yii::t('StoreModule', 'Название'),
			'url'                    => Yii::t('StoreModule', 'URL'),
			'price'                  => Yii::t('StoreModule', 'Цена'),
			'is_active'              => Yii::t('StoreModule', 'Активен'),
			'short_description'      => Yii::t('StoreModule', 'Краткое описание'),
			'full_description'       => Yii::t('StoreModule', 'Полное описание'),
			'meta_title'             => Yii::t('StoreModule', 'Meta Title'),
			'meta_keywords'          => Yii::t('StoreModule', 'Meta Keywords'),
			'meta_description'       => Yii::t('StoreModule', 'Meta Description'),
			'layout'                 => Yii::t('StoreModule', 'Макет'),
			'view'                   => Yii::t('StoreModule', 'Шаблон'),
			'sku'                    => Yii::t('StoreModule', 'Артикул')   ,
			'quantity'               => Yii::t('StoreModule', 'Количество'),
			'availability'           => Yii::t('StoreModule', 'Доступность'),
			'auto_decrease_quantity' => Yii::t('StoreModule', 'Автоматически уменьшать количество'),
			'created'                => Yii::t('StoreModule', 'Дата создания'),
			'updated'                => Yii::t('StoreModule', 'Дата обновления'),
			'discount'               => Yii::t('StoreModule', 'Скидка'),
			'main_category_id'       => Yii::t('StoreModule', 'Категория'),
		);
	}

  public static function getLastViewedList(){
    $list = Yii::app()->RediskaConnection->getConnection()->hGetAll(self::LastViewedList);
    if (!is_array($list)) {
      $list = array(0);
    }
    $list[] = 0;
  ...
  public function addToLastViewedList(){
    $list = self::getLastViewedList();
    if (!in_array($this->id, $list)){
      $list[] = $this->id;
    }
    $list = array_slice($list, -6, 6);
    Yii::app()->RediskaConnection->getConnection()->delete(self::LastViewedList);
    if (!Yii::app()->RediskaConnection->getConnection()->hMSet(self::LastViewedList, $list)){
      print_r(Yii::app()->RediskaConnection->getConnection()->getLastError());
    }
  }
    ...
    if (Yii::app()->user->getIsManager()) {
      $criteria->addInCondition($this->tableAlias.".market_id", Yii::app()->user->getUserMarketsIds());
    }
	...
	public function behaviors()
	{
		return array(
			...
			'comments' => array(
				...
				'class_name'  => Comment::CLASS_PRODUCT,
	...
	public function beforeSave()
	{
      ...
      if (Yii::app()->user->getIsManager()) {
        ...
        $this->market_id = array_pop($markets);
      } else {
        $this->market_id = StoreMarket::ADMIN_MARKET;
      }
		}
		else
		{
	...
	public static function getAvailabilityItems(){
		return array(
			1=>Yii::t('StoreModule', 'Есть на складе'),
			2=>Yii::t('StoreModule', 'Нет на складе'),
		);
	}
	...
	{
		$sort = new CSort;
		$sort->defaultOrder = false; //'t.created DESC';
