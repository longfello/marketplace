...
class StoreDeliveryMethod extends BaseModel
{
	...
	public function rules()
	{
		return array(
			array('name', 'required'),
			array('position, market_id', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
      ...
      'regions'        => array(self::HAS_MANY, 'StoreDeliveryRegion', array('delivery_id'=>'id')),
		);
	}
	...
	public function validatePaymentMethods($attr)
	{
		...
		foreach($this->$attr as $id)
		{
			if(StorePaymentMethod::model()->countByAttributes(array('id'=>$id)) == 0)
				$this->addError($attr, Yii::t('StoreModule', 'Ошибка проверки способка оплаты.'));
		}
	}
    ...
    if (!$this->regions) {
      ...
      $region->delivery_id = $this->id;
      $region->object_id   = 0;
      $region->type        = StoreDeliveryRegion::TYPE_ALL;
      $region->save();
    }
	...
	public function attributeLabels()
	{
		return array(
			...
			'free_from'        => Yii::t('StoreModule', 'Бесплатен от'),
			'active'           => Yii::t('StoreModule', 'Активен'),
			'description'      => Yii::t('StoreModule', 'Описание'),
			'position'         => Yii::t('StoreModule', 'Позиция'),
			'payment_methods'  => Yii::t('StoreModule', 'Способы оплаты'),
