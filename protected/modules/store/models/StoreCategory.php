...
class StoreCategory extends BaseModel
{
	...
	public function attributeLabels()
	{
		return array(
			...
			'lft'              => 'Lft',
			'rgt'              => 'Rgt',
			'level'            => Yii::t('StoreModule', 'Уровень'),
			'name'             => Yii::t('StoreModule', 'Название'),
			'url'              => Yii::t('StoreModule', 'URL'),
			'full_path'        => Yii::t('StoreModule', 'Полный путь'),
  ...
  public function calcProductsCount(){
    return 0;
    /*
    $productsCount = 0;
    $childs = StoreCategory::model()->findAll(
      array(
        'condition'=>'lft >= :lft AND rgt <= :rgt',
        'params'=>array(':lft' => intval($this->lft), ':rgt' => intval($this->rgt))
      )
    );
    foreach($childs as $one) {
      $productsCount += $one->countProducts;
    }
