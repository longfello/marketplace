...
class ProductsController extends SAdminController
{
	...
	public function actionUpdate($new = false)
	{
		...
		if($model->isNewRecord && isset($_GET['StoreProduct']['type_id']))
		{
			if(StoreProductType::model()->countByAttributes(array('id'=>$model->type_id)) === '0')
				throw new CHttpException(404, Yii::t('StoreModule', 'Ошибка. Тип продукта указан неправильно.'));
		}
		...
		$form->additionalTabs = array(
			Yii::t('StoreModule','Дополнительные категории') => $this->renderPartial('_tree', array('model'=>$model), true),
			Yii::t('StoreModule','Сопутствующие продукты') => $this->renderPartial('_relatedProducts',array(
				'exclude'=>$model->id,
				'product'=>$model,
			),true),
			Yii::t('StoreModule','Изображения')    => $this->renderPartial('_images', array('model'=>$model), true),
			Yii::t('StoreModule','Характеристики') => $this->renderPartial('_attributes', array('model'=>$model), true),
			Yii::t('StoreModule','Варианты')       => $this->renderPartial('_variations', array('model'=>$model), true),
			Yii::t('StoreModule','Отзывы')         => $this->renderPartial('_comments', array('model'=>$model), true),
		);

		if($model->use_configurations)
			$form->additionalTabs[Yii::t('StoreModule','Конфигурации')] = $this->renderPartial('_configurations', array('product'=>$model), true);
	...
	public function actionRenderVariantTable()
	{
		...
		if(!$attribute)
			throw new CHttpException(404, Yii::t('StoreModule', 'Ошибка загрузки атрибута'));
	...
	public function actionUpdateIsActive()
	{
		...
		echo Yii::t('StoreModule', 'Изменения успешно сохранены.');
	}
	...
	public function validateAttributes(StoreProduct $model)
	{
		...
		foreach($attributes as $attr)
		{
			...
			{
				$errors = true;
				$model->addError($attr->name, Yii::t('StoreModule', 'Поле %s обязательно для заполнения', array('%s'=>$attr->title)));
			}
		}
		...
		if($images && sizeof($images) > 0)
		{
			...
			foreach($images as $image)
			{
				if(!StoreUploadedImage::hasErrors($image))
					...
					$this->setFlashMessage(Yii::t('StoreModule', 'Ошибка загрузки изображения {name}', array('{name}'=>$image->getName())));
			}
		}
	}
}
