...
class ProductTypeController extends SAdminController {
	...
	public function actionUpdate($new = false)
	{
		...
		if (!$model)
			throw new CHttpException(404, Yii::t('StoreModule', 'Тип продукта не найден.'));
			...
			if (!empty($model))
			{
				foreach($model as $m)
				{
					if($m->productsCount > 0)
						throw new CHttpException(404, Yii::t('StoreModule', 'Ошибка удаления типа продукта. Он используется в продуктах.'));
					else
						$m->delete();
				}
			}
