...
class CurrencyController extends SAdminController {
	...
	public function actionUpdate($new = false)
	{
		...
		if (!$model)
			throw new CHttpException(404, Yii::t('StoreModule', 'Валюта не найдена.'));
	...
	public function actionDelete($id = array())
	{
		if (Yii::app()->request->isPostRequest)
		{
			...
			if (!empty($model))
			{
				...
				{
					if($m->main)
						throw new CHttpException(404, Yii::t('StoreModule', 'Ошибка. Удаление главной валюты запрещено.'));
					if($m->default)
						throw new CHttpException(404, Yii::t('StoreModule', 'Ошибка. Удаление валюты по умолчанию запрещено.'));
