...
class AttributeController extends SAdminController
{
	...
	public function actionUpdate($new = false)
	{
		...
		if (!$model)
			throw new CHttpException(404, Yii::t('StoreModule', 'Атрибут не найден.'));

		$form = new STabbedForm('application.modules.store.views.admin.attribute.attributeForm', $model);
		$form->additionalTabs = array(
			Yii::t('StoreModule', 'Опции')=>$this->renderPartial('_options', array(
				'model'=>$model,
			), true),
		);
		...
		if (Yii::app()->request->isPostRequest)
		{
			...
			if (!empty($model))
			{
				foreach($model as $m)
				{
					...
					if($count)
						throw new CHttpException(503, Yii::t('StoreModule', 'Ошибка удаления атрибута - он используется продуктами.'));
					$m->delete();
				}
			}
