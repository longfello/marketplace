...
<table class="optionsEditTable">
	<thead>
		<tr>
			<td></td>
			<?php foreach(Yii::app()->i18n->languages as $l): ?>
			<td>
				<?php echo CHtml::encode($l->name) ?>
			</td>
			<?php endforeach; ?>
			<td>
				<a href="#" class="plusOne"><?php echo Yii::t('StoreModule', 'Добавить') ?></a>
			</td>
		</tr>
	</thead>
	<tbody>
		<tr class="copyMe">
			...
			<?php endforeach; ?>
			<td>
				<a href="#" class="deleteRow"><?php echo Yii::t('StoreModule', 'Удалить') ?></a>
			</td>
		</tr>
		<?php
			if($model->options)
				...
				foreach($model->options as $o)
				{
					?>
						<tr>
							<td><span class="ui-icon ui-icon-arrowthick-2-n-s"></span></td>
							<?php
								foreach(Yii::app()->i18n->languages as $l):
