...
$title = ($model->isNewRecord) ? Yii::t('StoreModule', 'Создание продукта') :
	Yii::t('StoreModule', 'Редактирование продукта');

$this->breadcrumbs = array(
	'Home'=>$this->createUrl('/admin'),
	Yii::t('StoreModule', 'Продукты')=>$this->createUrl('index'),
	($model->isNewRecord) ? Yii::t('StoreModule', 'Создание продукта') : CHtml::encode($model->name),
);
	...
	if($model->isNewRecord && !$model->type_id || $attributeError === true)
	{
		...
		if($attributeError)
		{
			echo '<div class="errorSummary"><p>'.Yii::t('StoreModule', 'Необходимо исправить следующие ошибки:').'</p>
					<ul>
						<li>'.Yii::t('StoreModule','Выберите атрибуты для конфигурации продуктов.').'</li>
					</ul>
			</div>';
		}
		...
		echo CHtml::submitButton(Yii::t('StoreModule', 'Создать'), array('name'=>false));
		echo CHtml::closeTag('div');
		echo CHtml::endForm();
