 ...
 * @property StoreMarket $markets
 * @property StoreImportSources $sources
 */
...
public function rules()
{
    ...
    array('password', 'passwordStrength', 'strength'=>self::WEAK),
  ...
  public function passwordStrength($attribute,$params)
  {
    ...
    if ($params['strength'] === self::WEAK)
      $res = preg_match('/^(?=.*[a-zA-Z0-9]).{5,}$/', $this->$attribute);
    elseif ($params['strength'] === self::STRONG)
      $res = preg_match('/^(?=.*\d(?=.*\d))(?=.*[a-zA-Z](?=.*[a-zA-Z])).{5,}$/', $this->$attribute);

    if(!$res)
      $this->addError($attribute, Yii::t('UsersNodule','Your password is not strong enough!'));
  }
	...
	public function checkIfAvailable($attr)
	{
		...
		if($check>0)
			$this->addError($attr, Yii::t('UsersModule', '{attr} уже занят другим пользователем.', array('{attr}'=>$labels[$attr])));
	}
	...
	public function relations()
	{
      ...
      'markets'       => array(self::HAS_MANY, 'StoreMarket', 'user_id'),
      'sources'       => array(self::HAS_MANY, 'StoreImportSources', array('id'=>'market_id'),'through'=>'markets'),
		);
	}
	...
	{
		return array(
			'id'           => Yii::t('UsersModule', 'ID'),
			'username'     => Yii::t('UsersModule', 'Логин'),
			'password'     => Yii::t('UsersModule', 'Пароль'),
			'email'        => Yii::t('UsersModule', 'Email'),
			'created_at'   => Yii::t('UsersModule', 'Дата создания'),
			'last_login'   => Yii::t('UsersModule', 'Последний вход'),
			'login_ip'     => Yii::t('UsersModule', 'IP Адрес'),
			'new_password' => Yii::t('UsersModule', 'Новый пароль'),
			'discount'     => Yii::t('UsersModule', 'Персональная скидка'),
			'banned'       => Yii::t('UsersModule', 'Бан'),
			'verifyCode'   => Yii::t('UsersModule', 'Код проверки'),
		);
	}
