...
class CommentsController extends SAdminController
{
	...
	public function actionUpdate($id)
	{
		...
		if(!$model)
			throw new CHttpException(404, Yii::t('CommentsModule', 'Комментарий не найден'));
		...
		if (Yii::app()->request->isPostRequest)
		{
			...
			if($model->validate())
			{
				...
				$this->setFlashMessage(Yii::t('CommentsModule', 'Изменения успешно сохранены'));
	...
	public function actionUpdateStatus()
	{
		...
		if(!array_key_exists($status, Comment::getStatuses()))
			throw new CHttpException(404, Yii::t('CommentsModule', 'Ошибка проверки статуса.'));
