...
class InstallConfigureForm extends CFormModel
{
	...
	public function checkDbConnection()
	{
		if(!$this->hasErrors())
		{
			...
			try{
				...
				$this->addError('dbPassword', Yii::t('InstallModule','Ошибка подключения к БД'));
			}
		}
	}
	...
	{
		return array(
			'siteName'        => Yii::t('InstallModule', 'Название сайта'),
			'installDemoData' => Yii::t('InstallModule', 'Установить демонстрационные данные'),
			'dbHost'          => Yii::t('InstallModule', 'Хост'),
			'dbName'          => Yii::t('InstallModule', 'Название'),
			'dbUserName'      => Yii::t('InstallModule', 'Имя пользователя'),
			'dbPassword'      => Yii::t('InstallModule', 'Пароль'),
		);
	}
