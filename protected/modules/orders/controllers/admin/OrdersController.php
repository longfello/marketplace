...
class OrdersController extends SAdminController {
	...
	public function actionUpdate($new = false)
	{
      ...
      if (Yii::app()->user->getIsManager()) {
        ...
        $model->market_id = array_pop($markets);
      } else {
        $model->market_id = StoreMarket::ADMIN_MARKET;
      }
		}
		else
		...
		if(Yii::app()->request->isPostRequest)
		{
			...
			if($model->validate())
			{
				...
				$this->setFlashMessage(Yii::t('OrdersModule', 'Изменения успешно сохранены'));
    ...
    if (Yii::app()->user->getIsManager()) {
      $cr->addInCondition(Order::model()->tableAlias.".market_id", Yii::app()->user->getUserMarketsIds());
		...
		$this->render('update', array(
			'deliveryMethods' => StoreDeliveryMethod::model()->applyTranslateCriteria()->orderByName()->findAll($cr),
	...
	public function actionAddProduct()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if(!$product)
				throw new CHttpException(404, Yii::t('OrdersModule', 'Ошибка. Продукт не найден.'));
	...
	{
		throw new CHttpException(404, Yii::t('OrdersModule', 'Заказ не найден.'));
	}
}
