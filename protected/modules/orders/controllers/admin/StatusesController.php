...
class StatusesController extends SAdminController
{
	...
	public function actionUpdate($new = false)
	{
		...
		if (!$model)
			throw new CHttpException(404, Yii::t('OrdersModule', 'Статус не найден.'));
			...
			if (!empty($model))
			{
				foreach($model as $m)
				{
					if($m->countOrders() == 0 && $m->id != 1)
						...
						throw new CHttpException(409, Yii::t('OrdersModule','Ошибка удаления статуса. Он используется заказами.'));
				}
			}
