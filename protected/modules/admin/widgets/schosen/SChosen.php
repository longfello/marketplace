...
class SChosen extends CWidget {
	...
	public function setElements($elements)
	{
		...
		foreach($elements as $objectId)
		{
			$cs->registerScript($objectId.'Chosen', '
				$("#'.$objectId.'").chosen({no_results_text: "'.Yii::t('AdminModule', 'Ничего не найдено').'", width: "280px"});
			');
		}
	}
	...
	public function registerScripts()
	{
		...
		$cs = Yii::app()->getClientScript();
		$cs->registerCssFile($assetsUrl.'/chosen.min.css');
		$cs->registerScriptFile($assetsUrl.'/chosen.jquery.min.js');
	}
