(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        */
        ...
        function createEditorElement(id, src, photo) {
            var html = '<div class="photo-editor">';
            html += '<div class="preview"><img src="' + htmlEscape(src) + '" alt=""/></div>';
            html += '<div>';
            //if (opts.hasName) {
              //html += '<label for="photo_name_' + id + '">' + opts.nameLabel + ':</label><input type="text" name="photo[' + id + '][name]" class="input-xlarge" value="' + htmlEscape(name) + '" id="photo_name_' + id + '"/>';

              html += "<div class='form-group'>";
              html += "<label class='control-label'>"+opts.nameLabel+"</label>";
              html += "<ul class='nav nav-tabs'>";
              var first = true;
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<li class='active'><a href='#name-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                } else {
                  html += "<li><a href='#name-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                }
                first = false;
              });
              html += '</ul>';
              ...
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<div id='name-"+id+"-"+entry.code+"' class='tab-pane active'>";
                  html += '<input type="text" name="photo[' + id + '][name_'+entry.code+']" class="input-xlarge" value="' + htmlEscape(photo['name_'+entry.code]) + '"/>';
                  html += "</div>";
                } else {
                  html += "<div id='name-"+id+"-"+entry.code+"' class='tab-pane'>";
                  html += '<input type="text" name="photo[' + id + '][name_'+entry.code+']" class="input-xlarge" value="' + htmlEscape(photo['name_'+entry.code]) + '"/>';
                  html += "</div>";
                }
                first = false;
            ...
            //};
           //if (opts.hasDesc) {
              //html += '<label for="photo_description_' + id + '">' + opts.descriptionLabel + ':</label><textarea name="photo[' + id + '][description]" rows="3" cols="40" class="input-xlarge" id="photo_description_' + id + '">' + htmlEscape(description) + '</textarea>';
              html += "<div class='form-group'>";
              html += "<label class='control-label'>"+opts.descriptionLabel+"</label>";
              html += "<ul class='nav nav-tabs'>";
              var first = true;
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<li class='active'><a href='#desc-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                } else {
                  html += "<li><a href='#desc-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                }
                first = false;
              });
              html += '</ul>';
              ...
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<div id='desc-"+id+"-"+entry.code+"' class='tab-pane active'>";
                  html += '<textarea name="photo[' + id + '][description_'+entry.code+']" rows="3" cols="40" class="input-xlarge">' + htmlEscape(photo['description_'+entry.code]) + '</textarea>';
                  html += "</div>";
                } else {
                  html += "<div id='desc-"+id+"-"+entry.code+"' class='tab-pane'>";
                  html += '<textarea name="photo[' + id + '][description_'+entry.code+']" rows="3" cols="40" class="input-xlarge">' + htmlEscape(photo['description_'+entry.code]) + '</textarea>';
                  html += "</div>";
                }
                first = false;
            ...
            //};
              ...
              html += "<div class='form-group'>";
              html += "<label class='control-label'>Cсылка</label>";
              html += "<ul class='nav nav-tabs'>";
              var first = true;
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<li class='active'><a href='#link-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                } else {
                  html += "<li><a href='#link-"+id+"-"+entry.code+"' data-toggle='tab'>"+entry.name+"</a></li>";
                }
                first = false;
              });
              html += '</ul>';
              ...
              opts.lang.forEach(function(entry) {
                if (first) {
                  html += "<div id='link-"+id+"-"+entry.code+"' class='tab-pane active'>";
                  html += '<input type="text" name="photo[' + id + '][link_'+entry.code+']" class="input-xlarge" value="' + htmlEscape(photo['link_'+entry.code]) + '"/>';
                  html += "</div>";
                } else {
                  html += "<div id='link-"+id+"-"+entry.code+"' class='tab-pane'>";
                  html += '<input type="text" name="photo[' + id + '][link_'+entry.code+']" class="input-xlarge" value="' + htmlEscape(photo['link_'+entry.code]) + '"/>';
                  html += "</div>";
                }
                first = false;
        ...
        /*if (opts.hasName || opts.hasDesc)*/photoTemplate += '<span class="editPhoto btn btn-primary btn-mini"><i class="glyphicon glyphicon-pencil icon-white"></i></span> ';
        photoTemplate += '<span class="deletePhoto btn btn-danger btn-mini"><i class="glyphicon glyphicon-remove icon-white"></i></span>' +
        ...
        function editPhotos(ids) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                form.append(createEditorElement(id, src, photos_inf[id]));
            }
            if (l > 0)$editorModal.modal('show').removeClass('hide');
        }
        ...
        for (var i = 0, l = opts.photos.length; i < l; i++) {
            ...
            addPhoto(resp['id'], resp['preview'], resp['name_ru'], resp['description_ru'], resp['rank']);
            photos_inf[resp['id']] = opts.photos[i];
        }
    }
